...
def get_jst_now() -> datetime:
    """日本時間（JST）の現在日時を取得（tzinfoなし）"""
    # tzinfoなしで統一して返す（DB保存・比較の一貫性のため）
...
def create_session(user_id: int) -> str:
    ...
    【修正】expires_atはtzinfoなしのJST文字列で保存（比較時の一貫性確保）
    """
    ...
    # tzinfoなしで保存（validate_sessionの比較と統一）
...
def _parse_datetime_naive(dt_str: str) -> datetime:
    """
    DB保存済みのdatetime文字列をtzinfoなしのdatetimeに変換。

    【修正ポイント】
    旧バージョンでは get_jst_now() がtzinfo付きで返していたため、
    DBに "+09:00" や "JST" 付きの文字列が保存されている場合がある。
    fromisoformat() がtzinfo付きdatetimeを返した場合はtzinfoを除去して統一する。
    """
    try:
        dt = datetime.fromisoformat(dt_str)
        # tzinfo付きの場合はtzinfoを除去（naive datetimeに統一）
        if dt.tzinfo is not None:
            dt = dt.replace(tzinfo=None)
        return dt
    except Exception:
        # パース失敗時は現在時刻の1秒前（期限切れ扱いにしない安全側）
        return get_jst_now()
...
def validate_session(session_token: str, user_agent: str = None) -> Optional[Dict]:
    ...
    【修正】タイムゾーン比較バグを修正。
    expires_atとnow_jstを両方tzinfoなし（naive datetime）で比較する。
    """
    if not session_token:
        log_session_event(
            "VALIDATE_NO_TOKEN", 
            details="セッショントークンが空またはNone",
    ...
    try:
        ...
        if not user_row:
            log_session_event(
                ...
                session_token=session_token,
                details=f"セッションのユーザー(user_id={user_id})が存在しない",
                user_agent=user_agent
            )
        ...
        # 【修正】tzinfoなしに統一してから比較
        expires_at = _parse_datetime_naive(expires_at_str)
        now_jst = get_jst_now()  # 既にtzinfoなし

        if expires_at > now_jst:
            remaining_days = (expires_at - now_jst).days
            log_session_event(
                "VALIDATE_SUCCESS", 
                user_id=user_data['id'], 
                username=user_data['username'],
                session_token=session_token,
                details=f"検証成功。残り{remaining_days}日",
                user_agent=user_agent
            )
            return {
                'id': user_data['id'],
            ...
            log_session_event(
                ...
                username=user_data['username'],
                session_token=session_token,
                details=f"有効期限切れ。期限: {expires_at_str}",
                user_agent=user_agent
            )
            delete_session(session_token)
            return None
        ...
        log_session_event(
            "VALIDATE_ERROR", 
            session_token=session_token,
            details=f"予期しないエラー: {type(e).__name__}: {str(e)}",
            user_agent=user_agent
        )
        return None
...
def cleanup_expired_sessions():
    ...
    cursor = conn.cursor()
    # tzinfoなしの現在JST時刻で比較
    cursor.execute("DELETE FROM user_sessions WHERE expires_at < ?", (get_jst_now().isoformat(),))
    conn.commit()
    conn.close()
...
def update_user_password(user_id: int, new_password: str) -> bool:
    try:
        ...
        cursor.execute(
            ...
            (password_hash, salt, get_jst_now().isoformat(), user_id)
        )
        conn.commit()
        conn.close()
        return True
...
def update_user(user_id: int, display_name: str = None, role: str = None) -> bool:
    try:
        ...
        params.append(get_jst_now().isoformat())
...
def update_fee_setting(setting_key: str, setting_value: int) -> bool:
    try:
        ...
        cursor.execute(
            ...
            (setting_value, get_jst_now().isoformat(), setting_key)
        )
        conn.commit()
        conn.close()
        return True
...
def update_attendance(reservation_id: int, data: Dict):
    ...
    data['updated_at'] = get_jst_now().isoformat()
    ...
    if record_type in unique_types:
        ...
        if existing:
            cursor.execute('''
                ...
                WHERE id = ?
            ''', (details, get_jst_now().isoformat(), existing['id']))
        else:
            cursor.execute('''
                INSERT INTO care_records (reservation_id, record_type, record_time, details)
                VALUES (?, ?, ?, ?)
            ''', (reservation_id, record_type, get_jst_now().isoformat(), details))
        ...
        if existing:
            ...
            cursor.execute('''
                INSERT INTO care_records (reservation_id, record_type, record_time, details)
                VALUES (?, ?, ?, ?)
            ''', (reservation_id, record_key, get_jst_now().isoformat(), details))
...
def save_nap_check_log(reservation_id: int, nap_index: int, check_time: str, 
    ...
    if existing:
        ...
        ''', (arrow_direction, 1 if is_corrected else 0, staff_name, 
              get_jst_now().isoformat(), existing['id']))
    else:
        cursor.execute('''
